...
const AppStore = () => {
    ...
    const onClickTab=(eachTabId)=>{
      setActiveTabId(eachTabId);
    }

    const filteredApps=appsList.filter(eachApp=>{
      const isAppNameMatch=eachApp.appName.toLowerCase().includes(searchInput.toLowerCase());
      const isCategoryMatch=eachApp.category===activeTabId;
      return isAppNameMatch && isCategoryMatch;
    })     
        ...
        <div className="app-store">
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTab => (
              <li key={eachTab.tabId} className='tab-item'>
                <button type='button' className={`tab-button ${activeTabId===eachTab.tabId ? 'active':''}`} onClick={()=>onClickTab(eachTab.tabId)}>{eachTab.displayText}</button>
              </li>
            ))}
          </ul>
